...
SRG_ClearObjects:
		...
		move.l	#$FFF8FFD4,(v_scrposy_vdp).w		; set Y scroll positions		
...
SRG_DumpPal:
		...
		move.l	d0,(timer1).l			; reset SSRG timer/flags
		move.l	d0,(timer2).l			; ''
		move.l	d0,(timer3).l			; ''
...
SSRGScreen_Loop:
		...
		move.b	#$04,(v_vbla_routine).w
		jsr	(WaitForVBla).l				; hult til V-Blank begins
		addq.w	#$01,(timer1).l			; increase timer
		...
		andi.b	#btnStart,(v_jpadpress1).w 			; is Start button pressed on any controller?
		bne.s	SSRGScreen_Finish						; if so, branch
		andi.b	#btnABC+btnStart,(v_jpadpressP).w 				; is B button pressed on Pico_pad?
		bne.s	SSRGScreen_Finish						; if so, branch
...
SRG_ScrollFG:	
		...
		lea	(v_hscrolltablebuffer+$1CC).w,a1			; load the horizontal scroll buffer address
		moveq	#$00,d0					; clear d0
...
SRG_FinFGScroll:
		...
		lea	(v_hscrolltablebuffer).w,a1			; load the horizontal scroll buffer address
		move.w	(timer3).l,d0			; set BG position
...
SRG_DrawFG:
		...
		enable_ints
		lea	(v_ram_start).l,a4			; load map buffer space address to a4		
		lea	(vdp_data_port).l,a5				; load VDP data port address to a5
...
UpdateScrollPositions:
		...
		move.w	$08(a0),(timer3).l			; load X position as X scroll BG
		move.w	$0A(a0),d0				; load Y position
		neg.w	d0					; negate opposite direction
		;!@ move.w	d0,($FFFFF618).w			; save as Y scroll BG
		;!@ move.w	d0,($FFFFF618).w			; save as Y scroll BG
		move.w	d0,(v_bgscrposy_vdp).w			; save as Y scroll BG		
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
MapRamData:
		;!@ dc.l	$FFFF4000				; Map address
		dc.l	v_ram_start+$4000				; Map address
		dc.l	$61040003				; V-Ram address
		dc.l	$000B000B				; X and Y draw amount
		dc.l	$00000000				; null/spare
		;!@ dc.l	$FFFF4120				; ''		
		dc.l	v_ram_start+$4120				; ''
		dc.l	$60000003				; ''
		dc.l	$000F000F				; ''
		dc.l	$00000000				; ''
		;!@ dc.l	$FFFF4320				; ''
		dc.l	v_ram_start+$4320				; ''
		dc.l	$60000003				; ''
		dc.l	$00100010				; ''
		dc.l	$00000000				; ''
		;!@ dc.l	$FFFF4562				; ''
		dc.l	v_ram_start+$4562				; ''
		dc.l	$60000003				; ''
		dc.l	$000F000F				; ''
		dc.l	$00000000				; ''
; ---------------------------------------------------------------------------
...
MapScreen:
		...
		lea	(vdp_data_port).l,a6				; load VDP data port address to a6
...
; Pal_SSRG:	dc.w	$0000,$0400,$0800,$0C00,$0E00,$0EEE,$0AAA,$0888
		...
		; dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
Pal_SSRG:	binclude		"SSRG/SSRG.pal"
		even
; ---------------------------------------------------------------------------
...
MapLink_SSRG:	binclude	"SSRG/MapLink.kos"
		even
;!@
MapLink2_SSRG:	binclude	"SSRG/MapLink2.kos"
		even		
; ---------------------------------------------------------------------------
; ===========================================================================